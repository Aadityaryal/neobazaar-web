      ...
      <div className="page-container">
        <div className="mx-auto max-w-6xl space-y-4">
          <div className="card">
            ...
            <p className="text-secondary">Govern campaign lifecycle and policy compliance.</p>
          </div>
          ...
          <div className="space-y-3">
            {campaigns.length === 0 ? (
              <div className="card empty-state">No campaigns found.</div>
            ) : (
                ...
                <div key={campaign.campaignId} className="card">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {(["draft", "active", "paused", "ended"] as CampaignStatus[]).map((status) => (
                      <button
                        key={status}
                        className={campaign.status === status ? "btn-primary btn-sm" : "btn-secondary btn-sm"}
                        onClick={() => changeStatus(campaign.campaignId, status)}
                      >
                        {status}
                      </button>
                    ))}
